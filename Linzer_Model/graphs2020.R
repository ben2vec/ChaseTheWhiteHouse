...
plot_score <- function(state_abbr_vec, show_sim = FALSE, from = start_date){
    ...
    # Ugly hacks to force ggplot2::facet_wrap to order states by predicted Biden score.
    # Creating state_pos factor variables, with levels ordered by predicted Biden share.
    ...
    winprob <- pred %>% 
        ...
        arrange(state) %>% select(state, biden_win) %>% 
        mutate(biden_win = round(100*biden_win)) %>%
    ...
    g <- g + 
        ...
        geom_point(data = df[df$state %in% state_abbr_vec & df$t >= from,],
                   aes(x = t, y = 100*p_biden, alpha = -sqrt(p_biden*(1-p_biden)/n_respondents)),
        ...
        ylab("Biden Share of the Biden + Trump Vote (in %)") +
        xlab("") + 
...
ggplot(data.frame(alpha = 100*(inv_logit(alpha + 
    ...
    xlab("Estimated National/States Discrepancy of Biden Scores (in % Points)") +
...
ggplot() + 
    geom_histogram(data = data.frame(ev = result_ev_all_states), aes(ev, fill = ifelse(ev >= 270,"Biden","Trump")), binwidth = 1) + 
    ...
    ggtitle(paste("EV distribution - Pr(Biden wins) = ", round(mean(result_ev_all_states >= 270)*100, 0), "%", sep = "")) 
...
table_pred <- data.frame(pred[pred$t == all_t[length(all_t)], c("state","p")], 
                         pred[pred$t == election_day, c("p", "biden_win")])
colnames(table_pred) <- c("state", "p_now", "p_forecast", "biden_win")
...
datatable(table_pred[,c("state_name", "number_polls", "prior", "diffprior_now", "p_now", "p_forecast", "biden_win")],
          colnames = c("State", "Number of Polls", "Prior", "Current - Prior Difference", "Current Score", "Forecast Score", "Pr(Biden Wins)"),
          rownames = FALSE,
          options = list(dom = c('f t'), pageLength = nrow(table_pred))) %>%
    formatStyle('biden_win',
                backgroundColor=styleInterval(seq(.01,.99,.01), colors_red_to_blue(100))) %>%
    formatPercentage('biden_win', 0) %>%
...
ggplot(data = ex_df, aes(x=va, y=tx)) + 
    ...
    xlim(minmax) + 
    ylim(minmax) + 
    xlab("Biden Score (Virginia)") + 
    ylab("Biden Score (Texas)") + 
    facet_wrap(~ type)
...
remaining_days <- as.Date("2020-07-26", origin = "1970-01-01"):election_day
...
plot_data <- plot_data %>% 
    rename(state = variable, biden = value) %>% 
    group_by(state, sim) %>% 
    mutate(last = last(biden)) %>% 
    ungroup

ggplot(data = plot_data[plot_data$date >= Sys.Date(),])  +
    facet_grid(. ~ sim) + 
    geom_line(aes(x=as.Date(date), y=biden, group=state, color = last), alpha = .5, size = .2) + 
    scale_color_gradientn(colors = c("darkred", "red", "purple", "royalblue", "blue"), values = c(0,.4,.5,.8,1)) +
    guides(color = FALSE) +
    xlab("") + 
    ylab("Biden vote intentions (Simulation)") +
    geom_point(data = plot_data[plot_data$date == election_day,],
               aes(x=as.Date(date), y=biden, color = last), size = .4) + 
    geom_text(data = plot_data[plot_data$date == election_day,],
              aes(x = as.Date(date), y = biden, label = state), 
